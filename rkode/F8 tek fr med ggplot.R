...
axes_1 <- df %>% 
  ggplot(aes(x))+
  labs(title=expression(paste("Teknologisk fremgang 1: ", y == Z * x^a, " med økning i Z")), 
    x="Arbeidskraft",
    y="Produksjon")
...
figur_a <- axes_1 + 
  ...
  scale_color_manual(name = "Produktfunksjoner",
                     values = c("blue", "red"),
                     labels = c(expression(2*x^0.5), expression(4*x^0.5))
  ) +
...
figur_a +
  ...
  annotate("text",x=as.numeric(løs_1),y=-0.3, label=as.numeric(løs_1))+
  annotate("text",x=as.numeric(løs_2),y=-0.3, label=as.numeric(løs_2))
...
# La oss se på effekten av å øke a
...
figur_b <- axes_b + 
  ...
  scale_color_manual(name = "Produktfunksjoner",
                     values = c("blue", "red"),
                     labels = c(expression(2*x^0.5), expression(2*x^0.75))
  ) +
  geom_hline(yintercept =5)
figur_b
...
figur_b +
  ...
  annotate("text",x=as.numeric(løs_1),y=-0.3, label=as.numeric(løs_1))+
  annotate("text",x=as.numeric(løs_2b),y=-0.3, label=as.numeric(løs_2b))
